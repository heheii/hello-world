...
class MyapplicationKafkaResourceIT {
    ...
    void setup() {
        ...
        MyapplicationKafkaResource kafkaResource = new MyapplicationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/myapplication-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/myapplication-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
